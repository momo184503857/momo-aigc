...
defineProps<{ src?: string; title: string; loading?: boolean; externalPreview?: boolean; statusLabel?: string; progress?: number; selectable?: boolean; selected?: boolean }>()
...
<template>
  <article class="ds-result-tile" :aria-label="title" :data-selected="selectable && selected">
    ...
    <div class="ds-result-actions" aria-label="图片操作">
      <slot name="actions">
        <Button variant="ghost" size="icon-sm" :disabled="!src || loading" aria-label="下载" title="下载" @click="$emit('download')"><Download /></Button>
    ...
    <Dialog v-model:open="open"><DialogContent><DialogTitle class="sr-only">{{ title }}</DialogTitle><DialogDescription class="sr-only">图片预览</DialogDescription><img v-if="open" :src="src" :alt="title" class="ds-result-preview" /></DialogContent></Dialog>
  </article>
</template>
