...
// 按容器宽度适配，组件展厅卡片和侧栏也能使用紧凑分页。
const compact = computed(() => width.value < 360)
...
<template>
  <div v-if="!hideOnEmpty || total > 0" ref="container" class="flex min-w-0 max-w-full flex-wrap items-center justify-end gap-3">
    ...
    <nav class="flex min-w-0 max-w-full flex-wrap items-center justify-end gap-0.5" aria-label="分页">
      <Button
        variant="ghost"
        size="icon"
      ...
      <span v-if="compact" class="whitespace-nowrap px-1 text-sm tabular-nums" aria-live="polite">{{ currentPage }} / {{ pageCount }}</span>
      <template v-for="(item, index) in (compact ? [] : pages)" :key="index">
