...
<template>
  ...
  <div v-if="family === 'scroll-page'" class="ds-stack">
    ...
    <U.DsScrollPage title="列表页面" show-heading><template #filters><U.Input aria-label="页面筛选示例" /></template><p>滚动内容区域</p><template #footer><U.Button>保存（模拟）</U.Button></template></U.DsScrollPage>
  </div>
  ...
  <U.DsNavigationDock v-if="family === 'navigation-dock'" :items="[{path:'/create',title:'创作工作台',icon:PenLine,active:true,children:[{path:'/create/quick',title:'快速生图',icon:PenLine,active:true},{path:'/create/free',title:'自由生图',icon:Image}]},{path:'/results',title:'生图记录',icon:Image},{path:'/prompts',title:'提示词库',icon:BookOpen}]" @navigate="message = '导航事件：' + $event" />
  ...
  <U.DsActionBar v-else-if="family === 'action-bar'" :busy="state === 'loading'" :disabled="state === 'disabled'" :reason="state === 'disabled' ? '还需上传：模特图、衣服图' : '预计生成 2 张'" :reason-display="state === 'disabled' ? 'tooltip' : 'inline'" @submit="message = '生成动作已触发（模拟）'" />
