  ...
  <U.DsThemeProvider :mode="mode" :density="density">
    ...
    <U.DsPage title="UI 组件库">
      ...
      <template v-if="category === '设计基础' && !search">
        <div class="ds-grid"><U.DsSection title="01 / 品牌与语义"><div class="ds-palette"><div v-for="tone in tones" :key="tone.variable" class="ds-stack"><div class="ds-swatch" :style="{ background: `var(${tone.variable})` }" /><span class="ds-heading">{{ tone.name }}</span></div></div></U.DsSection><U.DsSection title="02 / 信息层级"><h2 class="ds-title">标题文字</h2><p class="ds-heading">小标题文字</p><p>正文文字 · 14px</p><p class="ds-caption">辅助文字 · 12px</p><div class="ds-row"><U.Button>主要操作<ArrowUpRight /></U.Button><U.Button variant="outline">次要操作</U.Button><U.Button variant="link">文字链接</U.Button></div></U.DsSection><U.DsSection title="03 / 空间与形状"><p>间距：4 / 8 / 12 / 16 / 20 / 24 / 32</p><p>基准圆角 8px · 卡片圆角 12px</p><p>舒适控件 36px · 紧凑控件 32px</p><div class="ds-panel">内容表面与细边框</div></U.DsSection></div>
      </template>
