...
const batchStripWidth = computed(() => Math.max(160, Math.min(clothImages.value.length + 1, 100) * 130 - 10))
          ...
          <div class="mb-2.5 flex flex-wrap items-center justify-between gap-2">
            <h3 class="flex items-center gap-1 text-sm font-semibold">
              ...
              <TooltipProvider :delay-duration="200">
                <Tooltip>
                  <TooltipTrigger as-child>
                    <Button variant="ghost" size="icon-xs" aria-label="模特脸图上传说明"><CircleHelp /></Button>
                  </TooltipTrigger>
                  <TooltipContent>必填 · 所有任务共用。源图五官将替换到每张衣服图的人脸上。</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </h3>
          </div>
