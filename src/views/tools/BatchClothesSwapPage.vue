 ...
 * 批量换姿势（模特图 × N  +  衣服图 × 1）
 *
 * 结构：素材区 = 单行模特图与紧邻的共用衣服图，吸底栏 = 可折叠摘要、公共参数与提交进度。
...
// 120px 缩略图 + 10px 间距；为共用衣服图及两区间距预留空间。
const modelStripWidth = computed(() => Math.max(160, Math.min(modelImages.value.length + 1, 100) * 130 - 10))
...
const blockingHint = computed(() => {
  ...
  if (modelImages.value.length === 0) return '请上传至少 1 张模特图'
  if (garmentImages.value.length === 0) return '请上传 1 张衣服图'
  return ''
})
...
<template>
  <PageLayout :dividers="false" inset-content>
    ...
    <div class="content-max flex min-w-0 flex-wrap items-start gap-x-8 gap-y-6">
      <!-- 模特图区按内容增长，到可用宽度上限后横向滚动。 -->
      <div class="min-w-0" :style="{ width: `min(100%, ${modelStripWidth}px)`, maxWidth: 'max(160px, calc(100% - 152px))' }">
        <!-- ① 模特图（批量源） -->
