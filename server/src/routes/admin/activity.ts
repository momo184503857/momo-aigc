...
adminActivityRouter.get('/', (req: AuthRequest, res) => {
  ...
  if (userId) {
    taskWhere += ' AND t.user_id = ?'
    taskParams.push(Number(userId))
  }
  ...
  if (userId) {
    txnWhere += ' AND pt.user_id = ?'
    txnParams.push(Number(userId))
  }
  ...
  if (type !== 'txn') {
    branches.push(`
      SELECT 'task' AS type, t.id AS id, t.user_id, u.username,
  ...
  if (type !== 'task') {
    branches.push(`
      SELECT 'txn' AS type, pt.id AS id, pt.user_id, u.username,
