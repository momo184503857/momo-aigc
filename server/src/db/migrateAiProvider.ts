...
function seedAiProvider(): void {
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(`
      ...
      VALUES ('toapis', 'ToAPIs', 'https://toapis.cn', 'toapis', '平台主渠道（迁移自共享 Key 体系）', 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    `).run()
...
function migrateUserKeys(): void {
  ...
  const tx = db.transaction(() => {
    const insertProvider = db.prepare(`
      ...
      VALUES (?, 'ToAPIs（迁移）', 'https://toapis.cn', 'toapis', '迁移自个人 ToAPIs Key', ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    `)
