...
最后更新：2026-06-15  
状态：已实现·后端已验证（curl）/ 前端已验证（类型检查 + 构造）/ key 新积分接口待接入
...
- Key 的新积分由**独立上游接口**提供（待接入，见 todo）。当前 `fetchKeyCredits()` 为占位：返回 ToAPIs CNY 余额 + `credits=null`，前端标注「新积分待接口」，**绝不**按 0.035 折算。
- ToAPIs 的 `credits`（`1 USD = 200 credits`）与平台新积分**无关**，UI 标注为「credits」。
...
| 我的额度 | `/my-quota` | 平台余额(¥) + 最近 10 条流水 + Key 额度（新积分待接口则显示 ToAPIs CNY 占位） |
...
- **待接入**：Key 的「新积分」由独立上游接口提供（当前 `fetchKeyCredits()` 占位）。
