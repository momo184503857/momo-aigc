...
1. **发布作品**：生成好的任务（`status='completed'`），用户在任务列表点「发布到作品库」。发布时从 `generation_tasks` 拷贝 prompt/参数/结果图，无需用户手填。标题默认取 prompt 前 30 字，可填描述和标签。
2. **作品展示**：作品库展示结果图、使用的模式、模型、提示词、参数、结构化字段、负面词。
3. **一键同款**：其他人浏览作品，点击「一键同款」复用全部参数跳转对应生图页面（复用 `sessionStorage('regenerate_task')` + `setParams()` 机制）。
4. **互动**：点赞、收藏、复用次数（比点赞更能体现价值的核心指标）。
5. **先发后审**：发布即公开，admin 可下架（`status='hidden'`）。
...
1. 三 Tab：作品广场 / 我的作品 / 我的收藏。
2. 筛选栏：模式下拉 + 标签筛选 + 关键词搜索 + 排序（最新/最热/最多复用）。
3. 瀑布流卡片网格：结果图 + 模式标签 + 模型 + 点赞/复用数 + 作者。
4. 点击卡片进详情。
...
1. 左侧：大图 + 作者信息 + 互动栏（点赞/收藏/一键同款/复用次数/浏览量）。
2. 右侧：完整参数面板--模式、模型、分辨率、宽高比、参考图缩略图、prompt 全文、结构化字段分段展示、负面词、标签。
...
- **作品库**（§9.8）：用户从已完成任务一键发布作品，展示结果图/模式/提示词/参数；其他人可浏览学习并「一键同款」复用参数。互动含点赞/收藏/复用次数。先发后审，admin 可上下架 + 发布官方种子。新增页面 `/works`（广场）、`/works/:id`（详情）、`/admin/works`（管理）。
