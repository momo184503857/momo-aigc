...
版本：v1.0  
日期：2026-05-25  
最后更新：2026-08-09（作品库重构：AI学习菜单组、瀑布流懒加载、卡片操作按钮、每日点赞、去标题、备注、详情页改版）  
项目类型：内部工具 / Web 应用  
...
1. 登录**独立的管理后台**（入口 `/admin.html`，与用户端入口分离；账号体系与用户端共用同一套 JWT，普通用户登录管理后台会被拒绝）。
2. 创建普通用户账号。
3. 重置普通用户密码。
4. 禁用 / 启用普通用户账号。
5. 查看所有用户的任务详情。
...
## 9.8 作品库（作品发布 + 学习他人 AI 思路）

### 解决什么问题

课程提到「拆解爆款」「沉淀关键词库」，本质是经验复用。但目前每个人生成完图，经验和提示词都散落在自己的任务历史里，别人看不到，无法互相学习。

### 功能设计

1. **发布作品**：生成好的任务（`status='completed'`），用户在任务列表点「发布到作品库」。发布时从 `generation_tasks` 拷贝 prompt/参数/结果图，无需用户手填。用户可选填备注和标签（无标题字段）。
2. **作品展示**：作品库展示结果图、提示词、参数、结构化字段、负面词、标签、备注。
3. **一键同款**：其他人浏览作品，点击「一键同款」复用全部参数跳转对应生图页面（复用 `sessionStorage('regenerate_task')` + `setParams()` 机制）。
4. **互动**：点赞（每人每天可对同一作品点赞一次，含自己的作品）、收藏、复用次数（比点赞更能体现价值的核心指标）。
5. **备注**：发布人和管理员可对作品添加/编辑备注（发布弹窗填写 + 详情页随时编辑）。
6. **先发后审**：发布即公开，admin 可下架（`status='hidden'`）。

### 冷启动

- admin 可发布官方种子作品（手动填参数 + 上传图，`is_official=1`）
- 官方种子 + 用户 UGC 混合展示，初期不空

### 风险

用户量太少，作品库可能做不起来、没内容。通过官方种子保底 + 一键同款降低复用门槛来缓解。

---

## 9.9 提示词工坊（结构化提示词 + 参考案例库）

### 解决什么问题

课程给出关键词六层权重公式（主体40% + 风格20% + 场景15% + 光影10% + 构图10% + 画质5%），结构化提示词能把可用率从 30% 提到 85%。但用户看到「柔光」「侧光」这类提示词，不知道会出现什么结果。提示词和效果之间没有可视化的对应关系，全靠盲猜。

### 功能设计

两部分联动，模块化去耦合：

1. **提示词结构化**（独立页面 `/prompt-workshop`，不改动生图表单）：
   - 把单行文本框改成按「主体/风格/场景/光影/构图/画质」六个分字段填写
   - 每个字段旁有「选词」按钮，弹出参考案例选择器
   - 按权重从大到小排序拼接成最终 prompt（词越靠前影响越大）
   - 支持负向规避词（negative prompt），以自然语言追加到 prompt 末尾（如「请避免出现：模糊、低质量」）
   - 实时预览拼接结果，可保存到提示词库（带 `segments` 字段）

2. **参考案例库**（看图选词）：
   - 针对每个结构化字段（尤其光影、风格），预先生成可视化参考图
   - 案例来源双轨：官方预生成（admin 维护 `prompt_cases` 表）+ 作品库聚合（`works` 中带该字段值的作品自动聚合）
   - 用户点关键词 -> 看参考图 -> 满意点「选用」-> 自动填入对应字段框

### 价值

把盲猜提示词变成看图选词。用户不需要懂光影理论，看到参考图效果对得上，直接复制即可。

### 技术约束

ToAPIs（gpt-image-2/gemini）是自然语言模型，不解析 SD 的 `(word:1.4)` 权重语法，也无 negative_prompt 字段。因此：
- 权重仅通过拼接顺序生效（不做括号语法输出）
- 负面词以自然语言追加到 prompt 文本末尾
- 结构化提示词是编辑态/检索态的价值，最终仍拼成单一 prompt 字符串发送
...
管理员后台是**独立于用户端的网页入口**，不再嵌入用户端侧边栏（用户端侧边栏对所有角色一律只展示 AI生图 / AI学习 / 资产管理三组，管理员菜单已整体移除）。

入口与账号：

- 入口 URL：`/admin.html`（用户端入口仍是 `/`）。
- 账号体系完全共用：同一套 JWT，同一个登录账号；在用户端登录过的管理员，打开 `/admin.html` 即为已登录态，无需重复登录。
- 管理后台有独立登录页，仅保留账号密码登录；登录后校验 `role === 'admin'`，普通用户登录被拒绝并提示「该账号无管理员权限」。
- 管理后台侧边栏提供「返回用户端」入口，一键跳回 `/`。

子页面（路径均为管理后台内部路由，hash 模式下形如 `/admin.html#/users`）：
...
8. API Key 管理 `/toapis-key`（共享 ToAPIs Key 配置 + 余额查询）。

> 用户端 `/#/admin/*` 路由作为兜底保留（无 UI 入口指向），仅用于历史链接/误访问时不白屏。
...
1. 范围切换：通过筛选栏下拉选择「作品广场 / 我的作品 / 我的收藏」（不再是独立 Tab）。
2. 筛选栏：范围下拉 + 模式下拉 + 标签筛选 + 关键词搜索（只搜 prompt）+ 刷新。
3. 排序栏（筛选栏下方独立一行）：最新 / 最热 / 最多复用。
4. 瀑布流懒加载：滚动到底部自动加载下一页（IntersectionObserver），无分页器。
5. 卡片内容：结果图 + 操作按钮行（赞 N / 收藏 / 同款 N / 复制提示词）+ 作者名；不显示标题、模式、模型、时间。
6. 卡片操作按钮常驻可见、可直接点击（蓝底白字 `el-button primary`），已赞/已收藏切 plain 状态。
7. 点击卡片图片进详情。
...
1. **左侧**：沉浸式大图（点击放大预览，接入 `useImagePreview`）+ 图下统计行（赞/收藏/复用/浏览数 + 时间）+ 操作按钮行（赞 / 收藏 / 一键同款 / 复制提示词）。
2. **右侧卡片式参数面板**（每段独立卡片 + 圆角阴影）：
   - 作者卡：头像 + 作者名 + 复用/浏览统计 + 「一键同款」按钮
   - 备注卡：发布人/管理员可编辑（橙色左边框区分），其他用户只读
   - 生成参数：模式 / 模型 / 分辨率 / 宽高比（2 列网格）
   - 参考图（可点击预览）
   - 提示词结构：六层字段分段展示，有内容的字段浅蓝底高亮
   - 完整提示词（标题右侧「复制」按钮）
   - 负向规避词（浅红底）
   - 标签
3. 「一键同款」按钮：调 `POST /api/works/:id/reuse` 记录复用 -> 拿参数 -> 写 `sessionStorage('regenerate_task')` -> 跳转对应生图页。
4. 删除按钮在 header 右上角（仅作者可见）。
5. 无标题字段（标题已移除）。
...
1. 六个分字段输入框（主体/风格/场景/光影/构图/画质），每个字段旁有「选词」按钮。
2. 负向规避词输入框。
3. 实时拼接预览（只读展示最终 prompt）。
4. 「选词」弹出参考案例选择器：该字段关键词列表 + 参考图缩略图，看图选词一键填入。
5. 保存到提示词库（带 `segments` 字段），在生图表单「从提示词库选择」时正常可用。

## 11.10 管理员：作品库管理 / 提示词案例管理

路径：`/admin/works`、`/admin/prompt-cases`

内容：

1. 作品库管理：表格列表（含已下架）、上架/下架、删除、发布官方种子作品、标签管理。
2. 提示词案例管理：官方案例图 CRUD（选字段 + 关键词 + 上传图 + 填 prompt 快照）。
...
### 2026-08-09 - 作品库 + 提示词工坊

- **作品库**（§9.8）：用户从已完成任务一键发布作品，展示结果图/提示词/参数；其他人可浏览学习并「一键同款」复用参数。互动含点赞（每人每天一次）/收藏/复用次数。广场为瀑布流懒加载，卡片常驻操作按钮。详情页沉浸式大图 + 卡片参数布局，支持备注（发布人/管理员可编辑）。无标题字段。先发后审，admin 可上下架 + 发布官方种子。新增页面 `/works`（广场）、`/works/:id`（详情）、`/admin/works`（管理）。侧边栏归入「AI学习」菜单组。
- **提示词工坊**（§9.9）：独立页面 `/prompt-workshop`，六层分字段（主体/风格/场景/光影/构图/画质）模块化拼装提示词 + 负面词自然语言追加。参考案例库「看图选词」，来源双轨（官方预生成 + 作品库聚合）。新增 `/admin/prompt-cases`（案例管理）。
- **数据库**：新增 6 张表（`works`/`work_tags`/`work_tag_relations`/`work_likes`/`work_favorites`/`prompt_cases`）+ 3 个迁移列（`prompt_library.segments`、`generation_tasks.prompt_segments`/`negative_prompt`）。详见 `docs/reference/database-schema.md`。
- **接口**：新增 `/api/works`、`/api/admin/works`、`/api/prompt-cases`、`/api/admin/prompt-cases` 四组路由。详见 `docs/reference/api-spec.md`。
- **两个功能去耦合**：工坊独立页面不改生图表单；作品库不强依赖结构化字段。通过 `prompt_segments` 软联动：工坊 -> 提示词库 -> 生图任务(快照) -> 发布作品 -> 案例库聚合。
