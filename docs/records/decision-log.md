...
---
## 2026-09-01 — 积分汇率 1:1 化：保留积分体系仅调汇率（非「废除积分改人民币」）

**背景**：积分与人民币双轨换算（1 积分 = ¥0.035）心智负担重：所有展示双显 `X 积分 (¥Y)`、管理端充值要折算、文档到处解释 0.035。用户最初提出「取消积分全部改人民币」。

**决策 — 保留积分计费体系，仅将汇率调为 1 积分 = ¥1，存量 ×0.035 幂等迁移**：表结构（`users.points` 等）、`/api/points/*` 端点、错误文案、「积分」措辞零改动；只改前后端 `YUAN_PER_CREDIT` 常量 + 一次性迁移 `migration_credits_v2`（`users.points` / `points_transactions` / `generation_tasks.points_*` / `ai_models.pricing` 全部 `ROUND(×0.035, 3)`，价值不变；迁移前 `VACUUM INTO` 自动备份）。**否决**「真改人民币」——需动全部表列、API 字段与文案，收益只是名字。展示收敛为积分单显（1:1 后两数值恒等，双显冗余）。种子定价常量保持旧单位由 v2 统一换算（新旧库冷启动路径都正确）；此后新增金额种子直接写新单位。推翻 2026-06-15 决策 3 中「1 新积分 = ¥0.035」的汇率（整数定价定价观随之变为 0.105 这类 3 位小数，管理端输入精度放宽）。
