...
- 范围切换通过筛选栏下拉选择（作品广场 / 我的作品 / 我的收藏），不再是独立 Tab。
- 筛选栏：搜索框（240px，只搜 prompt）+ 范围下拉 + 模式下拉（140px）+ 标签下拉（140px）+ 刷新按钮，`flex-wrap` 换行。
- 排序栏（筛选栏下方独立一行）：最新 / 最热 / 最多复用（`el-radio-group`）。
- 瀑布流用 CSS `column-count`（4/3/2 列响应式断点 1400px/1000px），`break-inside: avoid` 防割裂；懒加载（`IntersectionObserver` 无限滚动，无分页器）。
- 作品卡：图片 `object-fit: cover` + 底部操作按钮行（赞 N / 收藏 / 同款 N / 复制提示词 + 作者名靠右）。按钮常驻可见、蓝底白字（`el-button primary`），已赞/已收藏切 `plain`。**不显示标题、模式、模型、时间**。
- 官方作品左上角 `el-tag type="warning"` 徽标。

### 作品详情页（`/works/:id`）

- 沉浸式大图 + 卡片参数布局（左 480px 固定 / 右 flex:1），≤1000px 改为上下。
- 左栏：沉浸式大图（点击放大预览，接入 `useImagePreview`，hover 右下角浮出「点击查看大图」）+ 图下统计行（赞/收藏/复用/浏览数 + 时间）+ 操作按钮行（赞 / 收藏 / 一键同款 / 复制提示词）。
- 右栏：卡片式参数面板，每段独立 `.info-card`（圆角 `--momo-radius-lg` + 浅阴影 `--momo-shadow-sm` + 细边框）：
  - 作者卡：头像 + 作者名 + 复用/浏览统计 + 「一键同款」按钮
  - 备注卡：橙色左边框区分，发布人/管理员可见「编辑」按钮，查看态/编辑态切换
  - 生成参数：模式 / 模型 / 分辨率 / 宽高比（2 列网格）
  - 参考图（可点击预览）
  - 提示词结构：有内容的字段浅蓝底（`--momo-color-brand-subtle`）高亮
  - 完整提示词（标题右侧「复制」按钮）
  - 负向规避词（浅红底 `--momo-color-danger-subtle`）
  - 标签
- 删除按钮在 header 右上角（仅作者可见）。无标题字段。
